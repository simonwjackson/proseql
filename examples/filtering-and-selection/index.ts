	...
	products: [
		{
			...
			inStock: true,
			tags: ["typing", "ergonomic", "usb"],
			description: "Cherry MX switches with backlight",
		},
		{
			id: "p2",
			name: "Wireless Mouse",
			...
			description: "Bluetooth mouse with silent clicks",
		},
		{
			id: "p3",
			name: "USB-C Hub",
			...
			inStock: false,
			tags: ["usb", "portable"],
			description: "7-in-1 hub with HDMI output",
		},
		{
			id: "p4",
			name: "Standing Desk",
		...
		},
		{
			...
			inStock: true,
			tags: ["ergonomic", "adjustable"],
			description: "Single monitor mount with gas spring",
		},
		{
			id: "p6",
			name: "Desk Pad",
...
async function main() {
	...
	console.log(
		`category in [electronics, accessories]: ${inCategories.length} results`,
	);
	...
	console.log(
		`tags has all [ergonomic, adjustable]: ${hasAllTags.length} results`,
	);
	...
	console.log(
		`price >= 100 AND in stock: ${expensiveAndInStock.length} results`,
	);
	...
	console.log(
		`(electronics OR furniture) AND in stock: ${nested.length} results`,
	);
