 ...
 *
 * Usage:
 *   bun run examples/file-persistence.ts
 */
...
const config = {
	...
	books: {
		...
		relationships: {
			...
			genre: { type: "ref" as const, target: "genres" as const, foreignKey: "genreId" },
			reviews: { type: "inverse" as const, target: "reviews" as const, foreignKey: "bookId" },
			tags: { type: "inverse" as const, target: "tags" as const, foreignKey: "bookId" },
		},
	},
	genres: {
		schema: GenreSchema,
	...
	reviews: {
		...
		relationships: {
			book: { type: "ref" as const, target: "books" as const, foreignKey: "bookId" },
		},
	},
	series: {
		schema: SeriesSchema,
...
// ============================================================================
// Layer — Node.js file system + all 7 text format serializers
// ============================================================================
...
const program = Effect.gen(function* () {
	...
	if (existingAuthors.length > 0) {
		...
		// Genres (.json5)
		const scifi = yield* db.genres.create({ id: "sci-fi", name: "Science Fiction", description: "Speculative worlds and futures" })
		const cyber = yield* db.genres.create({ id: "cyberpunk", name: "Cyberpunk", description: "High tech, low life" })

		// Publishers (.toml)
		yield* db.publishers.create({ id: "ace", name: "Ace Books", founded: 1952 })
		yield* db.publishers.create({ id: "harper", name: "Harper Voyager", founded: 1817 })

		// Authors (.yaml)
		const herbert = yield* db.authors.create({ name: "Frank Herbert", born: 1920 })
		const leguin = yield* db.authors.create({ name: "Ursula K. Le Guin", born: 1929 })
		const gibson = yield* db.authors.create({ name: "William Gibson", born: 1948 })
		...
		// Books (.json)
		const dune = yield* db.books.create({ title: "Dune", authorId: herbert.id, year: 1965, genreId: scifi.id })
		const leftHand = yield* db.books.create({ title: "The Left Hand of Darkness", authorId: leguin.id, year: 1969, genreId: scifi.id })
		const neuro = yield* db.books.create({ title: "Neuromancer", authorId: gibson.id, year: 1984, genreId: cyber.id })
		const countZero = yield* db.books.create({ title: "Count Zero", authorId: gibson.id, year: 1986, genreId: cyber.id })

		// Reviews (.jsonc)
		yield* db.reviews.create({ bookId: dune.id, rating: 5, text: "The spice must flow" })
		yield* db.reviews.create({ bookId: dune.id, rating: 4, text: "Dense but rewarding" })
		yield* db.reviews.create({ bookId: neuro.id, rating: 5, text: "Invented the genre" })
		yield* db.reviews.create({ bookId: leftHand.id, rating: 5, text: "Masterpiece of imagination" })
	...
	console.log(`\n${stats.count} books, ${reviewStats.count} reviews, ${tagStats.count} tags`)

	// Flush writes to disk
	yield* Effect.promise(() => db.flush())

	console.log("\nData saved to 7 different formats:")
