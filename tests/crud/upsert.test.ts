	...
	beforeEach(async () => {
		...
		db = await Effect.runPromise(
			createEffectDatabase(config, {
				users: [
					{ id: "user1", email: "john@example.com", username: "johndoe", name: "John Doe", age: 30, loginCount: 5, lastLoginAt: yesterday, bio: "Software developer", tags: ["developer"], companyId: "comp1", createdAt: now, updatedAt: now },
					{ id: "user2", email: "jane@example.com", username: "janesmith", name: "Jane Smith", age: 25, loginCount: 3, tags: ["designer"], companyId: "comp2", createdAt: now, updatedAt: now },
				],
				products: [
					{ id: "prod1", sku: "LAPTOP-001", name: "Laptop Pro", description: "High-end laptop", price: 1999.99, stock: 10, lastRestocked: yesterday, categoryId: "cat1", createdAt: now, updatedAt: now },
					{ id: "prod2", sku: "MOUSE-001", name: "Wireless Mouse", description: "Ergonomic mouse", price: 49.99, stock: 100, categoryId: "cat2", createdAt: now, updatedAt: now },
				],
				settings: [
					{ id: "set1", userId: "user1", settingKey: "theme", value: "dark", createdAt: now, updatedAt: now },
					{ id: "set2", userId: "user1", settingKey: "language", value: "en", createdAt: now, updatedAt: now },
					{ id: "set3", userId: "user2", settingKey: "theme", value: "light", createdAt: now, updatedAt: now },
				],
				companies: [
					{ id: "comp1", name: "TechCorp", domain: "techcorp.com", employeeCount: 100, createdAt: now, updatedAt: now },
					{ id: "comp2", name: "DataInc", domain: "datainc.io", employeeCount: 50, createdAt: now, updatedAt: now },
				],
				categories: [
					{ id: "cat1", name: "Computers", slug: "computers", description: "Computer products", createdAt: now, updatedAt: now },
					{ id: "cat2", name: "Accessories", slug: "accessories", description: "Computer accessories", createdAt: now, updatedAt: now },
				],
			}),
		)
	})
	...
	describe("upsert method (single entity)", () => {
		describe("upsert by ID", () => {
			...
			it("should update existing entity when ID found", async () => {
				...
				const result = await db.users.upsert({
					where: { id: "user1" },
					create: { email: "new@example.com", username: "newuser", name: "New User", age: 40, companyId: "comp1" },
		...
		describe("upsert by unique field", () => {
			it("should create by unique email when not found", async () => {
				const result = await db.users.upsert({
					...
					create: { email: "newuser@example.com", username: "newuser", name: "New User", age: 28, companyId: "comp2" },
			...
			it("should update by unique email when found", async () => {
				const result = await db.users.upsert({
					...
					create: { email: "john@example.com", username: "johnnew", name: "John New", age: 31, companyId: "comp1" },
					update: { age: 31, bio: "Senior developer", tags: { $append: "senior" } },
				}).runPromise
			...
			it("should upsert by SKU for products", async () => {
				const result1 = await db.products.upsert({
					...
					create: { sku: "LAPTOP-001", name: "New Laptop", description: "Brand new", price: 2499.99, stock: 5, categoryId: "cat1" },
					update: { price: 1799.99, stock: { $increment: 5 }, lastRestocked: new Date().toISOString() },
				}).runPromise
				...
				const result2 = await db.products.upsert({
					...
					create: { sku: "LAPTOP-002", name: "Laptop Air", description: "Lightweight", price: 1299.99, stock: 20, categoryId: "cat1" },
		...
		describe("upsert with validation", () => {
			it("should validate foreign keys in create", async () => {
				const error = await Effect.runPromise(
					db.users.upsert({
						where: { email: "newuser@example.com" },
						create: { email: "newuser@example.com", username: "newuser", name: "New User", age: 30, companyId: "invalid-company" },
						update: { loginCount: { $increment: 1 } },
					}).pipe(Effect.flip),
				)
			...
			it("should validate foreign keys in update", async () => {
				const error = await Effect.runPromise(
					db.products.upsert({
						where: { id: "prod1" },
						create: { sku: "NEW-001", name: "New Product", description: "New", price: 99.99, stock: 10, categoryId: "cat1" },
		...
		describe("upsert edge cases", () => {
			it("should handle create and update with different shapes", async () => {
				const result = await db.categories.upsert({
					...
					create: { name: "New Category", slug: "new-category", description: "A brand new category" },
			...
			it("should preserve timestamps correctly", async () => {
				...
				const result = await db.users.upsert({
					where: { id: "user1" },
					create: { email: "new@example.com", username: "newuser", name: "New User", age: 30, companyId: "comp1" },
	...
	describe("upsertMany method (batch upsert)", () => {
		describe("basic batch upsert", () => {
			it("should handle mixed create and update operations", async () => {
				const result = await db.products.upsertMany([
					{
						where: { sku: "LAPTOP-001" },
						create: { sku: "LAPTOP-001", name: "Laptop Pro", description: "High-end", price: 2499.99, stock: 0, categoryId: "cat1" },
						update: { price: 1899.99, stock: { $increment: 10 } },
					},
					{
						where: { sku: "TABLET-001" },
						create: { sku: "TABLET-001", name: "Tablet Pro", description: "Professional tablet", price: 999.99, stock: 15, categoryId: "cat1" },
						update: { price: 899.99 },
					},
					{
						where: { sku: "MOUSE-001" },
						create: { sku: "MOUSE-001", name: "Basic Mouse", description: "Basic", price: 29.99, stock: 0, categoryId: "cat2" },
			...
			it("should handle all creates", async () => {
				const result = await db.categories.upsertMany([
					...
					{
						where: { slug: "software" },
						create: { name: "Software", slug: "software", description: "Software products" },
						update: { description: "Updated" },
					},
				]).runPromise
			...
			it("should handle all updates", async () => {
				...
				const result = await db.users.upsertMany([
					{
						where: { email: "john@example.com" },
						create: { email: "john@example.com", username: "john", name: "John", age: 30, companyId: "comp1" },
						update: { loginCount: { $increment: 1 }, lastLoginAt: loginTime },
					},
					{
						where: { email: "jane@example.com" },
						create: { email: "jane@example.com", username: "jane", name: "Jane", age: 25, companyId: "comp2" },
						update: { loginCount: { $increment: 1 }, lastLoginAt: loginTime },
					},
				]).runPromise
				...
				const result = await db.companies.upsertMany([
					{
						...
						create: { name: "TechCorp", domain: "techcorp.com", employeeCount: 100 },
						update: { employeeCount: 100 },
					},
					{
						where: { domain: "datainc.io" },
						create: { name: "DataInc", domain: "datainc.io", employeeCount: 50 },
		...
		describe("batch upsert error handling", () => {
					...
					db.users.upsertMany([
						{
							...
							create: { email: "valid@example.com", username: "validuser", name: "Valid User", age: 30, companyId: "comp1" },
							update: { age: 31 },
						},
						{
			...
			it("should handle foreign key validation", async () => {
				const error = await Effect.runPromise(
					db.products.upsertMany([
						{
							where: { sku: "VALID-001" },
							create: { sku: "VALID-001", name: "Valid", description: "V", price: 99.99, stock: 10, categoryId: "cat1" },
							update: { stock: 15 },
						},
						{
							where: { sku: "INVALID-001" },
							create: { sku: "INVALID-001", name: "Invalid", description: "I", price: 99.99, stock: 10, categoryId: "invalid-cat" },
							update: { stock: 15 },
						},
					]).pipe(Effect.flip),
				)
				...
				const result = await db.users.upsertMany([
					{
						...
						create: { email: "john@example.com", username: "johndoe", name: "John Doe", age: 30, companyId: "comp1" },
						update: { loginCount: { $increment: 1 }, tags: { $append: ["active", "verified"] }, bio: { $set: "Updated bio" } },
					},
					{
						where: { username: "newuser" },
						create: { email: "new@example.com", username: "newuser", name: "New User", age: 22, tags: ["new"], companyId: "comp2" },
						update: { loginCount: { $increment: 1 }, tags: { $append: "active" } },
					},
				]).runPromise
