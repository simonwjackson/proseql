...
const ProductSchema = Schema.Struct({
	...
	tags: Schema.optional(Schema.Array(Schema.String), { default: () => [] as ReadonlyArray<string> }),
	features: Schema.optional(Schema.Array(Schema.String), { default: () => [] as ReadonlyArray<string> }),
				...
				users: [
					{
						...
						bio: "Software developer", tags: ["developer", "senior"], score: 100,
						isActive: true, companyId: "comp1", createdAt: now, updatedAt: now,
					},
					{
						id: "user2", name: "Jane Smith", email: "jane@example.com", age: 25,
						tags: ["designer"], score: 50, isActive: true, companyId: "comp2",
						createdAt: now, updatedAt: now,
					},
					{
						id: "user3", name: "Bob Johnson", email: "bob@example.com", age: 35,
						tags: ["manager", "senior"], score: 75, isActive: false, companyId: "comp1",
						createdAt: now, updatedAt: now,
					},
				],
				companies: [
					{
						id: "comp1", name: "TechCorp", employeeCount: 100, revenue: 1000000,
						tags: ["tech", "startup"], isPublic: false, createdAt: now, updatedAt: now,
					},
					{
						id: "comp2", name: "DataInc", employeeCount: 50, revenue: 500000,
						tags: ["data", "ai"], isPublic: true, createdAt: now, updatedAt: now,
					},
				],
				products: [
					{
						id: "prod1", name: "Laptop Pro", description: "High-end laptop", price: 1999.99,
						stock: 10, soldCount: 5, tags: ["electronics", "computers"],
						features: ["16GB RAM", "512GB SSD"], isAvailable: true, categoryId: "cat1",
						createdAt: now, updatedAt: now,
					},
					{
						id: "prod2", name: "Wireless Mouse", description: "Ergonomic wireless mouse",
						price: 49.99, stock: 100, soldCount: 50, tags: ["accessories"],
						features: ["Bluetooth", "Rechargeable"], isAvailable: true, categoryId: "cat2",
						createdAt: now, updatedAt: now,
					},
				],
				categories: [
					{ id: "cat1", name: "Computers", description: "Desktop and laptop computers", createdAt: now, updatedAt: now },
					{ id: "cat2", name: "Accessories", description: "Computer accessories", createdAt: now, updatedAt: now },
				],
			}),
		)
	})
	...
	describe("update method (single entity)", () => {
		describe("basic updates", () => {
			it("should update a single field", async () => {
				...
				const result = await db.users.update("user1", { name: "John Updated" }).runPromise
				...
				const updatedUser = allUsers.find((u: Record<string, unknown>) => u.id === "user1")
			...
			it("should auto-update updatedAt timestamp", async () => {
				...
				const result = await db.users.update("user1", { score: 110 }).runPromise
		...
		describe("update operators", () => {
			...
			describe("array operators", () => {
				...
				it("should append multiple items to array", async () => {
					...
					expect(result.tags).toEqual(["developer", "senior", "lead", "architect"])
				})
				...
				it("should prepend items to array", async () => {
					...
					expect(result.features).toEqual(["Touchscreen", "16GB RAM", "512GB SSD"])
				})
		...
		describe("error handling", () => {
			it("should return NotFoundError for non-existent entity", async () => {
				const error = await Effect.runPromise(
					db.users.update("non-existent", { name: "Updated" }).pipe(Effect.flip),
				)
			...
			it("should not allow updating immutable fields", async () => {
				const error = await Effect.runPromise(
					db.users.update("user1", { id: "new-id" } as Record<string, unknown>).pipe(Effect.flip),
				)

				expect(error._tag).toBe("ValidationError")

				const error2 = await Effect.runPromise(
					db.users.update("user1", { createdAt: "2024-01-01T00:00:00.000Z" } as Record<string, unknown>).pipe(Effect.flip),
				)
			...
			it("should validate foreign key constraints", async () => {
				const error = await Effect.runPromise(
					db.users.update("user1", { companyId: "non-existent-company" }).pipe(Effect.flip),
				)
		...
		describe("relationship updates", () => {
			it("should update relationship field with valid foreign key", async () => {
				const result = await db.users.update("user1", { companyId: "comp2" }).runPromise
			...
			it("should validate new foreign key exists", async () => {
				const error = await Effect.runPromise(
					db.products.update("prod1", { categoryId: "cat-invalid" }).pipe(Effect.flip),
				)
	...
	describe("updateMany method (batch update)", () => {
		describe("basic batch updates", () => {
			it("should update all matching entities", async () => {
				...
				expect(result.updated.every((u) => u.bio === "TechCorp employee")).toBe(true)
			})
			...
			it("should update all entities when predicate always true", async () => {
				...
				expect(result.count).toBe(2)
				expect(result.updated.every((c) => c.tags?.includes("2024"))).toBe(true)
			})
		})
		...
		describe("batch updates with operators", () => {
			...
			it("should handle array operations in batch", async () => {
				...
				expect(result.count).toBe(2)
				expect(result.updated.every((u) => u.tags?.includes("experienced"))).toBe(true)
		...
		describe("batch update special cases", () => {
			it("should update entities matching OR-like conditions", async () => {
				...
				expect(result.count).toBe(2)
				expect(result.updated.map((u) => u.id).sort()).toEqual(["user2", "user3"])
			})
