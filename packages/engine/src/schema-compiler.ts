...
import { getCollectionConfigs, isSourceOrientedDatabaseConfig } from "@proseql/core";
import type { Schema } from "effect";
import { Effect } from "effect";
import { mergeGlobalHooks } from "./plugin-registry.js";
...
export interface CallbackRegistrar {
	...
	registerCustomOperator(name: string, supportedTypes: ReadonlyArray<string>, evaluate: (field: unknown, operand: unknown) => boolean): Promise<void>;
...
export const compileDatabaseDescriptor = async <Config extends DatabaseConfig>(
	...
	for (const operator of plugins.operators.values()) {
		await registrar.registerCustomOperator(operator.name, operator.types, operator.evaluate);
	}
	for (const generator of plugins.idGenerators.values()) {
		registrar.registerIdGenerator(generator.name, generator.generate);
	}

	const collections = Object.entries(getCollectionConfigs(config)).map(([name, collection]) =>
...
function compileCollectionDescriptor(
	...
	return {
		name,
		schema: compileStructLikeSchema(collection.schema, `${name}.schema`, registrar),
		...
		before_create_hooks: registerHookArray(
			hooks.beforeCreate as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerBeforeCreateHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.beforeCreate`,
		),
		after_create_hooks: registerHookArray(
			hooks.afterCreate as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerAfterCreateHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.afterCreate`,
		),
		before_update_hooks: registerHookArray(
			hooks.beforeUpdate as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerBeforeUpdateHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.beforeUpdate`,
		),
		after_update_hooks: registerHookArray(
			hooks.afterUpdate as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerAfterUpdateHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.afterUpdate`,
		),
		before_delete_hooks: registerHookArray(
			hooks.beforeDelete as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerBeforeDeleteHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.beforeDelete`,
		),
		after_delete_hooks: registerHookArray(
			hooks.afterDelete as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerAfterDeleteHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.afterDelete`,
		),
		on_change_hooks: registerHookArray(
			hooks.onChange as ReadonlyArray<(ctx: any) => Effect.Effect<unknown>> | undefined,
			(prefix, hook) => registrar.registerOnChangeHook((ctx) => runEffect(hook(ctx as never)), prefix),
			`${name}.onChange`,
		),
		computed_fields: Object.entries(
			(collection.computed ?? {}) as Record<string, (entity: unknown) => unknown>,
		).map(([fieldName, callback]) => ({
			name: fieldName,
			callback_id: registrar.registerComputed(callback, `${name}.computed.${fieldName}`),
		})),
		search_index: collection.searchIndex ? [...collection.searchIndex] : [],
		...(collection.idGenerator ? { id_generator: collection.idGenerator } : {}),
		...(collection.version !== undefined ? { version: collection.version } : {}),
		migrations: ((collection.migrations ?? []) as ReadonlyArray<Migration>).map((migration, index) =>
			compileMigration(name, migration, index, registrar),
		),
...
function compileIdStrategy(collection: CollectionConfig): Record<string, unknown> {
...
function mergeHooks<T>(
	...
	const merge = <A>(a: ReadonlyArray<A> | undefined, b: ReadonlyArray<A> | undefined) => {
		const values = [...(a ?? []), ...(b ?? [])].filter((value) => value !== undefined);
		return values.length > 0 ? values : undefined;
	};
	return {
		beforeCreate: merge(globalHooks?.beforeCreate, collectionHooks?.beforeCreate),
		afterCreate: merge(globalHooks?.afterCreate, collectionHooks?.afterCreate),
		beforeUpdate: merge(globalHooks?.beforeUpdate, collectionHooks?.beforeUpdate),
		afterUpdate: merge(globalHooks?.afterUpdate, collectionHooks?.afterUpdate),
		beforeDelete: merge(globalHooks?.beforeDelete, collectionHooks?.beforeDelete),
...
function registerHookArray<T>(
	...
	return (hooks ?? []).map((hook: T, index) => register(`${prefix}.${index}`, hook));
}
...
function compileStructLikeSchema(
	...
	if (ast?._tag === "Transformation" && ast.from?._tag === "TypeLiteral" && ast.to?._tag === "TypeLiteral") {
		return compileTypeLiteralFields(anySchema.fields ?? {}, path, registrar);
	}
	if (ast?._tag === "TypeLiteral" && anySchema.fields) {
		return compileTypeLiteralFields(anySchema.fields, path, registrar);
	}
	throw new Error(`Unsupported root schema at '${path}': expected Schema.Struct, got ${describeAst(ast)}`);
}
...
function compileTypeLiteralFields(
	...
	return {
		...
		fields: Object.entries(fields).map(([name, fieldSchema]) => ({
			name,
			schema: compileFieldSchema(fieldSchema as any, `${path}.${name}`, registrar),
		})),
	};
}
	...
	if (context?.isOptional) {
		if (typeof context.defaultValue === "function") {
			return {
				...
				defaultCallbackId: registrar.registerDefault(context.defaultValue, `${path}.default`),
			};
		}
		return { kind: "optional", inner: compileSchemaNode(inner, path, registrar) };
	}
	if (ast?._tag === "PropertySignatureDeclaration") {
		if (ast.isOptional) {
			return { kind: "optional", inner: compileSchemaNode(schema.from, path, registrar) };
		}
	...
	if (ast?._tag === "PropertySignatureTransformation") {
		...
		if (typeof defaultValue === "function") {
			return {
				...
				defaultCallbackId: registrar.registerDefault(defaultValue, `${path}.default`),
			};
		}
...
function compileSchemaNode(
	...
	switch (ast?._tag) {
		...
		case "StringKeyword":
			return { kind: "str" };
		case "Number":
			if (Array.isArray(ast.encoding) && ast.encoding.some((entry: any) => entry.to?._tag === "String")) {
		...
		case "TypeLiteral": {
			...
			if (ast.propertySignatures.length === 0 && ast.indexSignatures.length === 1) {
				...
				return {
					kind: "record",
					key: compileSchemaNode({ ast: indexSignature.parameter }, `${path}.[key]`, registrar),
					value: compileSchemaNode({ ast: indexSignature.type }, `${path}.[value]`, registrar),
				};
			}
			if (ast.propertySignatures.length === 0 && ast.indexSignatures.length === 0) {
				return { kind: "struct", fields: [] };
			}
			throw new Error(`Unsupported type literal at '${path}': mixed property/index signatures are not supported`);
		}
		case "Transformation": {
			if (ast.from?._tag === "StringKeyword" && ast.to?._tag === "NumberKeyword") {
				return { kind: "numFromStr" };
			}
			if (ast.from?._tag === "TypeLiteral" && ast.to?._tag === "TypeLiteral" && schema.fields) {
				return compileTypeLiteralFields(schema.fields, path, registrar);
			}
			throw new Error(`Unsupported transformation at '${path}': ${describeAst(ast.from)} -> ${describeAst(ast.to)}`);
		}
		case "Null":
		...
		case "Union": {
			const nullMembers = ast.types.filter(
				(member: any) =>
					(member._tag === "Literal" && member.literal === null) || member._tag === "Null",
			);
			const undefinedMembers = ast.types.filter(
				(member: any) => member._tag === "UndefinedKeyword" || member._tag === "Undefined",
			);
			...
			const literalValues = ast.types
				...
				.filter((value: unknown): value is string | number | boolean | null => value !== undefined);
			if (literalValues.length === ast.types.length && literalValues.length > 0) {
				return literalValues.length === 1
					? { kind: "literal", value: literalValues[0] }
					: { kind: "literalUnion", values: literalValues };
			}
			if (nullMembers.length === 1 && undefinedMembers.length === 0 && otherMembers.length === 1) {
				return {
					kind: "nullOr",
					inner: compileSchemaNode({ ast: otherMembers[0] }, path, registrar),
				};
			}
			if (undefinedMembers.length === 1 && nullMembers.length === 0 && otherMembers.length === 1) {
				return {
					kind: "optional",
					inner: compileSchemaNode({ ast: otherMembers[0] }, path, registrar),
				};
			}
			throw new Error(`Unsupported union at '${path}': only null/undefined unions and literal unions are supported`);
		}
		case "Literal": {
		...
		case "Declaration":
		default:
			throw new Error(`Unsupported schema combinator at '${path}': ${describeAst(ast)}`);
	}
}
...
function literalValueFromAst(ast: any): string | number | boolean | null | undefined {
	if (ast?._tag === "Literal") {
...
function isAsyncEffectSuspension(error: unknown): boolean {
	...
	return /synchronous|runSync|async/i.test(message) && /effect|fiber/i.test(message);
}
