...
try {
	for (const file of allFiles) {
		if (expectedXfails.has(file)) {
			results.push(
				await runRunnableFile(file, "xfail", expectedXfails.get(file)),
			);
...
const xfailedResults = xfailResults.filter(
...
const report = {
	...
	summary: {
		...
		coverageRate:
			allFiles.length === 0
				? 0
				: (runnableResults.length + xfailResults.length) / allFiles.length,
		executedFraction:
			allFiles.length === 0
				? 0
				: (runnableResults.length + xfailResults.length) / allFiles.length,
...
async function runRunnableFile(file, expectation, expected = undefined) {
	...
	writeFileSync(
		transformedPath,
		transformCoreTest(readFileSync(sourcePath, "utf8"), file),
	);
	...
	return {
		...
		category:
			expectation === "xfail"
				? (expected?.category ?? "known-failure")
				: passed
					? undefined
					: "engine-bug",
...
function transformCoreTest(source, fileName = "") {
	let transformed = source
		.replace(
			...
			(_match, quote, subpath) => {
				if (
					subpath === "factories/database-effect" ||
					subpath === "index" ||
					subpath === "errors/index"
				) {
		...
		.replace(/\b_as ([^;]+);/g, "as $1;")
		.replace(
			/([A-Za-z0-9_]+)\.pipe\(\s*\(chunk\) => Array\.from\(chunk\)\[0\],\s*\)/g,
			"Array.from($1)[0]",
		)
		.replace(
			/Effect\.timeoutFail\(\{\s*duration:\s*"([^"]+)",\s*onTimeout:\s*\(\) => new Error\("[^"]*"\),\s*\}\)/g,
			'Effect.timeout("$1")',
		)
		.replace(
			/([a-z][A-Za-z0-9_]*Scope)\.fork\(([^)]+)\)/g,
			"Scope.fork($1, $2)",
		)
		.replaceAll(
			"ExecutionStrategy.sequential",
			'({ _tag: "Sequential" } as any)',
		)
	...
	if (transformed.includes("openPersistentDatabase(")) {
		transformed = transformed.replace(
			/import \{([^}]*)\} from "effect";/,
			(_match, imports) =>
				`import {${ensureNamedImport(imports, ["Exit", "Scope"])} } from "effect";`,
		);
		transformed = transformed.replace(
			/import \{([^}]*)\} from "vitest";/,
			(_match, imports) =>
				`import {${ensureNamedImport(imports, ["afterAll"])} } from "vitest";`,
		);
...
function classifySkip(file) {
	...
	const typeOnly =
		/(?:-types|type-safety|query-config-types|crud-methods-types)\.test\.ts$/.test(
			file,
		);
	if (typeOnly) {
		return {
			...
			reason:
				"Type-only or compile-time assertion suite; covered separately by the adapter's dedicated typecheck gate.",
		};
	}
	...
	if (internalPrimitive) {
		return {
			...
			owner: "effect-adapter",
			reason:
				"Targets internal core primitives or helper modules rather than the factory-swappable adapter surface exercised by the U9 parity slice.",
		};
	}
	...
	return {
		...
		owner: "effect-adapter",
		reason:
			"Adapter-facing suite not yet transformed into this phase-2 slice inventory. Keep explicitly tracked until the parity slice expands further.",
	};
}
