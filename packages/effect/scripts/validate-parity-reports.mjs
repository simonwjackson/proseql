...
const corpus = JSON.parse(
	readFileSync(join(reportDir, "corpus-report.json"), "utf8"),
);
const examples = JSON.parse(
	readFileSync(join(reportDir, "examples-report.json"), "utf8"),
);
...
function validateCorpusReport(report) {
	...
	const failing = report.results.filter(
		(result) => result.expectation === "pass" && result.status !== "passed",
	);
	const unexpectedXpass = report.results.filter(
		(result) => result.expectation === "xfail" && result.status === "xpassed",
	);
	const sliceSkips = report.results.filter(
		(result) => result.category === "slice-not-yet-adapted",
	);
	if (failing.length > 0) {
		throw new Error(
			`Corpus report contains runnable failures: ${failing.map((result) => result.test).join(", ")}`,
		);
	}
	if (unexpectedXpass.length > 0) {
		throw new Error(
			`Corpus report contains unexpected xpasses: ${unexpectedXpass.map((result) => result.test).join(", ")}`,
		);
	}
	if (sliceSkips.length > 0) {
		throw new Error(
			`Corpus report still contains slice-not-yet-adapted entries: ${sliceSkips.map((result) => result.test).join(", ")}`,
		);
	}
}
...
function validateExamplesReport(report) {
	...
	requireNumber(
		report.summary.runnableExamples,
		"examples.summary.runnableExamples",
	);
	...
	if (failing.length > 0) {
		throw new Error(
			`Examples report contains runnable failures: ${failing.map((result) => result.test).join(", ")}`,
		);
	}
	if (unexpectedXpass.length > 0) {
		throw new Error(
			`Examples report contains unexpected xpasses: ${unexpectedXpass.map((result) => result.test).join(", ")}`,
		);
	}
}
