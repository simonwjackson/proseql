...
type SynchronousAfterHookFindById = (id: string) =>
...
const loadNodeEngineModule = (): Promise<EffectEngineModule> => {
	if (!nodeEngineModulePromise) {
		nodeEngineModulePromise = import(
			"@proseql/engine"
		) as Promise<EffectEngineModule>;
	}
	return nodeEngineModulePromise;
};

const loadBrowserEngineModule = (): Promise<EffectEngineModule> => {
	if (!browserEngineModulePromise) {
		browserEngineModulePromise = import(
			"@proseql/engine/browser"
		) as Promise<EffectEngineModule>;
	}
...
const withStreamRunPromise = <A, E>(
	...
	Object.defineProperty(stream, "runPromise", {
		get() {
			if (cached === undefined) {
				cached = Effect.runPromise(Stream.runCollect(stream)) as Promise<
					ReadonlyArray<A>
				>;
			}
			return cached;
...
const makeCloseOnce = (
	...
	plugins:
		| ReadonlyArray<{
				readonly shutdown?: () => Effect.Effect<unknown, unknown, never>;
		  }>
		| undefined,
	flush?: () => Promise<void>,
) => {
...
const engineOptionsFrom = (
	options: EffectDatabaseOptions | undefined,
): EngineDatabaseOptions | undefined =>
	options ? { plugins: options.plugins } : undefined;

const collectionNamesFromConfig = (
...
const makePersistenceOptions = (
	...
	return {
		...
		...(serializerRegistry
			? {
					serializerRegistry,
					_persistObjectFile: (path: string, data: unknown, format: string) =>
						Effect.runPromise(
							Effect.gen(function* () {
		...
		storageHost: {
			...
			write: (path: string, data: string) =>
				Effect.runPromise(adapter.write(path, data)),
			append: (path: string, data: string) =>
				Effect.runPromise(adapter.append(path, data)),
			...
			listDirectory: (dirPath: string) =>
				Effect.runPromise(adapter.listDirectory(dirPath)),
			listRecursive: (rootPath: string) =>
				Effect.runPromise(adapter.listRecursive(rootPath)),
			watch: (path: string, onChange: () => void) =>
				Effect.runPromise(adapter.watch(path, onChange)),
			watchDir: (
				dirPath: string,
				onChange: (event: {
...
const streamQuery = (
	...
	config?: QueryConfig<any, any, any>,
) =>
...
const cursorOrStreamQuery = (
	engineCollection: EngineCollection<any, any, any>,
	config?: QueryConfig<any, any, any>,
) => {
	if (config && "cursor" in config && config.cursor !== undefined) {
		return withRunPromise(
			liftPromise(() => engineCollection.query(config as never)),
		) as never;
	}
...
const extractFirstCauseFailure = (
	cause: unknown,
):
...
const optionsAfterPredicateLimit = (options?: {
...
const createTransactionCollectionAdapter = (
	...
	findById: (id: string) =>
		wrapTransactionOperation(state, () =>
			engineCollection.findById(id),
		) as never,
	exists: (id: string) =>
		wrapTransactionOperation(state, () => engineCollection.exists(id)) as never,
	create: (input: unknown) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.create(input as never),
			() => {
	...
	createMany: (
		...
		wrapTransactionOperation(
			state,
			() => engineCollection.createMany(inputs as never, options),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	update: (id: string, updates: unknown) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.update(id, updates as never),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	...
	delete: (id: string, options?: { readonly soft?: boolean }) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.delete(id, options),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	...
	upsert: (input: unknown) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.upsert(input as never),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	upsertMany: (inputs: ReadonlyArray<unknown>) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.upsertMany(inputs as never),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	createWithRelationships: (input: unknown) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.createWithRelationships(input as never),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	updateWithRelationships: (id: string, input: unknown) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.updateWithRelationships(id, input as never),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	deleteWithRelationships: (
		id: string,
		options?: DeleteWithRelationshipsOptions<any, any>,
	) =>
		wrapTransactionOperation(
			state,
			() => engineCollection.deleteWithRelationships(id, options as never),
			() => {
				state.mutatedCollections.add(collectionName);
			},
		) as never,
	deleteManyWithRelationships: (
		...
		wrapTransactionOperation(
			...
			async () => {
				if (typeof predicate !== "function") {
					return engineCollection.deleteManyWithRelationships(
						predicate as never,
						options as never,
					);
				}
	...
	aggregate: (config: unknown) =>
		wrapTransactionOperation(state, () =>
			engineCollection.aggregate(config as never),
		) as never,
...
const createCollectionAdapter = (
	engineCollection: EngineCollection<any, any, any>,
	transactionRunner: <A>(
		fn: (tx: EngineCollection<any, any, any>) => Promise<A>,
	) => Promise<A>,
) => ({
	query: (config?: QueryConfig<any, any, any>) =>
		cursorOrStreamQuery(engineCollection, config),
	findById: (id: string) => {
	...
	exists: (id: string) =>
		withRunPromise(liftPromise(() => engineCollection.exists(id))) as never,
	create: (input: unknown) => {
	...
	update: (id: string, updates: unknown) =>
		withRunPromise(
			liftPromise(() => engineCollection.update(id, updates as never)),
		) as never,
	updateMany: (predicate: (entity: any) => boolean, updates: unknown) => {
		const operation = () => {
			if (typeof predicate !== "function") {
	...
	delete: (id: string, options?: { readonly soft?: boolean }) =>
		withRunPromise(
			liftPromise(() => engineCollection.delete(id, options)),
		) as never,
	deleteMany: (
		predicate: (entity: any) => boolean,
		options?: { readonly soft?: boolean; readonly limit?: number },
	...
	upsert: (input: unknown) =>
		withRunPromise(
			liftPromise(() => engineCollection.upsert(input as never)),
		) as never,
	upsertMany: (inputs: ReadonlyArray<unknown>) =>
		withRunPromise(
			liftPromise(() => engineCollection.upsertMany(inputs as never)),
		) as never,
	createWithRelationships: (input: unknown) =>
		withRunPromise(
			liftPromise(() =>
		...
		withRunPromise(
			liftPromise(() =>
				transactionRunner(async (txCollection) => {
					if (typeof predicate !== "function") {
						return txCollection.deleteManyWithRelationships(
							predicate as never,
							options as never,
						);
					}
	...
	aggregate: (config: unknown) =>
		withRunPromise(
			liftPromise(() => engineCollection.aggregate(config as never)),
		) as never,
	watch: (config?: unknown) =>
		subscriptionEffectToStream(() =>
			engineCollection.watch(config as never),
		) as never,
	watchById: (id: string, options?: { readonly debounceMs?: number }) =>
		subscriptionEffectToStream(() =>
			engineCollection.watchById(id, options),
		) as never,
});
...
const adaptDatabase = <Config extends DatabaseConfig>(
	...
	const transactionRunner = <A>(
		...
		engineDb.$transaction(async (tx) =>
			fn(
				(tx as Record<string, EngineCollection<any, any, any>>)[
					collectionName
				]!,
			),
		);
	...
	const adapted = Object.assign(collections, {
		$transaction: <A, E>(
			...
			withRunPromise(
				Effect.tryPromise({
					try: (signal) =>
						engineDb.$transaction(async (tx) => {
							const txRecord = tx as Record<
								string,
								EngineCollection<any, any, any>
							>;
							...
							for (const name of collectionNames) {
								txCollections[name] = createTransactionCollectionAdapter(
									txRecord[name]!,
									name,
									state,
								);
									...
									value: () =>
										withRunPromise(
											Effect.suspend(() => {
												if (!state.active) {
													return Effect.fail(
														transactionInactiveError("commit"),
													);
												}
												state.active = false;
									...
									value: () =>
										withRunPromise(
											Effect.suspend(() => {
												if (!state.active) {
													return Effect.fail(
														transactionInactiveError("rollback"),
													);
												}
												state.active = false;
							...
							const typedTxContext = txContext as TransactionContext<
		...
		$documentGraph: {
			getRecordProvenance: (collection: string, id: string) =>
				liftPromise(() =>
					engineDb.$documentGraph.getRecordProvenance(collection, id),
				) as never,
			getDiagnostics: () =>
				liftPromise(() => engineDb.$documentGraph.getDiagnostics()) as never,
		},
	...
	Object.defineProperty(adapted, "__proseqlMaterializationDiagnostics", {
		value: () =>
			(
				engineDb as unknown as {
					__proseqlMaterializationDiagnostics?: () => unknown;
				}
...
const adaptPersistentDatabase = <Config extends DatabaseConfig>(
	...
	const db = adaptDatabase(
		engineDb,
		collectionNames,
	) as GenerateDatabaseWithPersistence<Config> & { close: () => Promise<void> };
	return Object.assign(db, {
		flush: () => engineDb.flush(),
		pendingCount: () => engineDb.pendingCount(),
		$dryRunMigrations: () =>
			withRunPromise(liftPromise(() => engineDb.$dryRunMigrations())) as never,
		close: () => engineDb.close(),
	});
};
...
const createEffectDatabaseWithLoader =
	...
	<Config extends DatabaseConfig>(
		...
		let defectConstructor:
			| EffectEngineModule["WasmEngineDefectError"]
			| undefined;
		return liftPromise<GenerateDatabase<Config>, CreateEffectDatabaseError>(
			...
			(error) =>
				defectConstructor !== undefined && error instanceof defectConstructor,
		);
	};
...
const createPersistentEffectDatabaseWithLoader =
	...
	> => {
		...
		return Effect.gen(function* () {
			...
			const serializerRegistryOption = yield* Effect.serviceOption(
				SerializerRegistryService,
			);
			const enginePersistence = makePersistenceOptions(
				adapterOption._tag === "Some" ? adapterOption.value : undefined,
				serializerRegistryOption._tag === "Some"
					? serializerRegistryOption.value
					: undefined,
				persistenceConfig,
			);
			...
			yield* Effect.addFinalizer(() =>
				Effect.promise(() => close()).pipe(Effect.orDie),
			);
			return db;
...
export const createEffectDatabase =
	createEffectDatabaseWithLoader(loadNodeEngineModule);
export const createPersistentEffectDatabase =
	createPersistentEffectDatabaseWithLoader(loadNodeEngineModule);
export const createBrowserEffectDatabase = createEffectDatabaseWithLoader(
	loadBrowserEngineModule,
);
export const createBrowserPersistentEffectDatabase =
	createPersistentEffectDatabaseWithLoader(loadBrowserEngineModule);

export const unsafeLiftPromiseForTests = liftPromise;
export const unsafeSubscriptionEffectToStreamForTests =
	subscriptionEffectToStream;
