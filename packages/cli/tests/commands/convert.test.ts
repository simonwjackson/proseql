  ...
  year: 1984`;
	...
	describe("multiple collections", () => {
		it("should only convert the specified collection", async () => {
			...
			fs.writeFileSync(
				authorsFilePath,
				JSON.stringify({ a1: { id: "a1", name: "Frank Herbert" } }, null, 2),
			);
			...
			expect(fs.existsSync(path.join(tempRoot, "data", "books.yaml"))).toBe(
				true,
			);
			expect(fs.existsSync(path.join(tempRoot, "data", "books.json"))).toBe(
				false,
			);

			// Authors should be unchanged
			expect(fs.existsSync(path.join(tempRoot, "data", "authors.json"))).toBe(
				true,
			);
		});
	});
