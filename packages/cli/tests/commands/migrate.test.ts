...
describe("Migrate Command", () => {
	...
	describe("detectSubcommand", () => {
		...
		it("should handle extra args after status", () => {
			expect(detectSubcommand(["status", "extra", "args"], false)).toBe(
				"status",
			);
		});
	});
	...
	describe("migrate run", () => {
		...
		it("should update status to up-to-date after successful migration", async () => {
			...
			const booksCollection = result.data?.collections.find(
				(c) => c.name === "books",
			);
			expect(booksCollection?.status).toBe("up-to-date");
			expect(booksCollection?.currentVersion).toBe(2);
  ...
  year: 1984`;
	...
	describe("multiple collections", () => {
		it("should report status for all versioned collections", async () => {
			...
			const _AuthorSchemaV1 = Schema.Struct({
				id: Schema.String,
				name: Schema.String,
			});
			...
			const booksStatus = result.data?.collections.find(
				(c) => c.name === "books",
			);
			const authorsStatus = result.data?.collections.find(
				(c) => c.name === "authors",
			);
