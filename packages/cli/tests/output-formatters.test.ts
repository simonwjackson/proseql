	...
	describe("formatAsTable", () => {
		...
		describe("column alignment", () => {
			it("should align columns based on header and data widths", () => {
				...
				const headerNameEnd = lines[0].indexOf("name") + "name".length
				const separatorNameEnd = lines[1].indexOf("  ")
		...
		describe("truncation", () => {
			...
			it("should truncate long headers with ellipsis", () => {
				const result = formatAsTable(
					[{ thisIsAVeryLongFieldName: "value" }],
					{ maxColumnWidth: 15 },
				)
				const lines = result.split("\n")
			...
			it("should handle very small maxColumnWidth", () => {
				const result = formatAsTable([{ name: "Alice" }], { maxColumnWidth: 3 })
				const lines = result.split("\n")
