...
describe("CRUD error creation and _tag discrimination", () => {
	it("NotFoundError has correct _tag and fields", () => {
		const err = new NotFoundError({ collection: "users", id: "123", message: "not found" })
	...
	it("ValidationError has correct _tag and issues", () => {
		const err = new ValidationError({
			...
			issues: [
				...
				{ field: "age", message: "must be a number", expected: "number", received: "string" },
			],
		})
...
describe("_tag discrimination in union types", () => {
	it("discriminates CRUD errors by _tag", () => {
		const errors = [
			...
			new DuplicateKeyError({ collection: "users", field: "email", value: "a@b.com", existingId: "2", message: "dup" }),
			new ValidationError({ message: "invalid", issues: [] }),
		]

		const tags = errors.map((e) => e._tag)
		expect(tags).toEqual(["NotFoundError", "DuplicateKeyError", "ValidationError"])
	})

	it("discriminates across all error categories", () => {
		const errors = [
			new NotFoundError({ collection: "users", id: "1", message: "not found" }),
			new DanglingReferenceError({ collection: "users", field: "companyId", targetId: "x", message: "dangling" }),
			new StorageError({ path: "/data", operation: "write", message: "failed" }),
		]

		const tags = errors.map((e) => e._tag)
		expect(tags).toEqual(["NotFoundError", "DanglingReferenceError", "StorageError"])
	})
})
...
describe("Effect.catchTag pattern matching", () => {
	...
	it("catches DuplicateKeyError and lets other errors through", async () => {
		const makeFailing = (tag: "dup" | "notfound") =>
			tag === "dup"
				? Effect.fail(new DuplicateKeyError({
						collection: "users", field: "email", value: "a@b.com", existingId: "2", message: "dup",
					}))
				: Effect.fail(new NotFoundError({ collection: "users", id: "1", message: "not found" }))
		...
		const uncaught = makeFailing("notfound").pipe(
			Effect.catchTag("DuplicateKeyError", () => Effect.succeed("should not happen")),
		)
	...
	it("catches StorageError", async () => {
		const effect = Effect.fail(
			new StorageError({ path: "/data/users.yaml", operation: "read", message: "ENOENT" }),
		).pipe(
	...
	it("chains multiple catchTag for different error types", async () => {
		const failing: Effect.Effect<string, NotFoundError | ValidationError> = Effect.fail(
			new ValidationError({ message: "bad", issues: [{ field: "x", message: "required" }] }),
		)

		const effect = failing.pipe(
			Effect.catchTag("NotFoundError", () => Effect.succeed("was not found")),
			Effect.catchTag("ValidationError", (err) => Effect.succeed(`validation: ${err.issues.length}`)),
		)
		const result = await Effect.runPromise(effect)
...
describe("errors are instances of Error", () => {
	it("TaggedErrors extend Error", () => {
		const err = new NotFoundError({ collection: "users", id: "1", message: "not found" })
		expect(err).toBeInstanceOf(Error)
	})

	it("TaggedErrors have a stack trace", () => {
		const err = new StorageError({ path: "/data", operation: "read", message: "failed" })
