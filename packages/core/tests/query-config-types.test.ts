...
import { Schema, Stream, Effect, Chunk } from "effect"
import type {
	QueryConfig,
	QueryReturnType,
	WhereClause,
...
describe("QueryConfig types with Effect Schema entities", () => {
	...
	describe("SelectConfig", () => {
		...
		it("should support array-based selection on Schema-inferred fields", () => {
			const arraySelect: SelectConfig<User, {}, {}> = ["name", "email"] as const
	...
	describe("QueryReturnType", () => {
		it("should return RunnableStream (Stream with .runPromise)", () => {
			...
			type _check1 = Assert<IsAssignable<PlainResult, Stream.Stream<User, unknown>>>

			// Verify it has runPromise
			type _check2 = Assert<IsAssignable<PlainResult, { runPromise: Promise<ReadonlyArray<User>> }>>
