...
describe("createEffectDatabase", () => {
	...
	describe("findById", () => {
		...
		it("should fail with NotFoundError for nonexistent id", async () => {
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					const db = yield* createEffectDatabase(config, initialData);
					return yield* db.users.findById("nonexistent").pipe(
	...
	describe("runPromise convenience API", () => {
		...
		it("query().runPromise with filter should return filtered array", async () => {
			...
			const users = await db.users.query({ where: { age: { $gt: 28 } } })
				.runPromise;
...
describe("createPersistentEffectDatabase", () => {
	describe("persistence hooks", () => {
			...
			await Effect.runPromise(
				Effect.scoped(
					Effect.gen(function* () {
						...
						const values = Object.values(parsed) as Array<
				...
				Effect.scoped(
					Effect.gen(function* () {
						...
						const parsed = JSON.parse(
							trackingStore.get("/data/companies.json")!,
						);
		...
		it("existing non-persistent createEffectDatabase still works unchanged", async () => {
			...
			const result = await Effect.runPromise(
				db.companies.create({ name: "OldAPI" }),
			);
