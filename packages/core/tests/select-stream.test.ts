...
describe("applySelect Stream combinator", () => {
  ...
  const users = [
    ...
    { id: "u3", name: "Charlie", email: "charlie@example.com", age: 35, active: true },
  ];
  ...
  describe("object-based selection", () => {
    ...
    it("should ignore fields not present in the item", async () => {
      const result = await collectSelected(users, { name: true, nonExistent: true });
      expect(result).toEqual([
        { name: "Alice" },
        { name: "Bob" },
        { name: "Charlie" },
  ...
  describe("nested selection on populated relationships", () => {
    ...
    it("should include full populated object when selection is true", async () => {
      ...
      expect(result).toEqual([
        { name: "Alice", company: { id: "c1", name: "Acme Corp", revenue: 1000000 } },
        { name: "Bob", company: { id: "c2", name: "Beta Inc", revenue: 500000 } },
      ]);
    });
    ...
    it("should apply nested selection to populated array", async () => {
      ...
      expect(result).toEqual([
        { name: "Alice", orders: [{ id: "o1", total: 100 }, { id: "o2", total: 200 }] },
  ...
  describe("edge cases", () => {
    ...
    it("should handle items with null/undefined field values", async () => {
      ...
      const result = await collectSelected(items, { name: true, company: true });
      expect(result).toEqual([
