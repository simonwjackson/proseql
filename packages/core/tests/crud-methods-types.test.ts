...
describe("CrudMethods returns Effect-based types", () => {
	...
	it("update returns RunnableEffect with NotFoundError", () => {
		...
		type _check = Assert<
			IsAssignable<
				UpdateReturn,
				Effect.Effect<User, ValidationError | NotFoundError | ForeignKeyError>
			>
		>;
	...
	it("delete returns RunnableEffect with OperationError", () => {
		...
		type _check = Assert<
			IsAssignable<
				DeleteReturn,
				Effect.Effect<User, NotFoundError | OperationError | ForeignKeyError>
			>
		>;
	...
	it("upsert returns RunnableEffect", () => {
		...
		type _check = Assert<
			IsAssignable<
				UpsertReturn,
				{
					readonly runPromise: Promise<
						User & { __action: "created" | "updated" }
					>;
				}
			>
		>;
...
describe("CrudMethodsWithRelationships extends CrudMethods with relationship operations", () => {
	...
	it("createWithRelationships returns RunnableEffect", () => {
		...
		type _check = Assert<
			IsAssignable<
				CreateWithRelsReturn,
				Effect.Effect<User, ValidationError | ForeignKeyError | OperationError>
			>
		>;
	...
	it("deleteWithRelationships returns RunnableEffect", () => {
		...
		type _check = Assert<
			IsAssignable<
				...
				Effect.Effect<
					{ deleted: User; cascaded?: Record<string, unknown> },
	...
	it("deleteManyWithRelationships returns RunnableEffect", () => {
		type Methods = CrudMethodsWithRelationships<User, UserRelations>;
		type DeleteManyWithRelsReturn = ReturnType<
			Methods["deleteManyWithRelationships"]
		>;
	...
	it("all methods have runPromise convenience property", () => {
		...
		type _check1 = Assert<
			IsAssignable<
				ReturnType<Methods["create"]>,
				{ readonly runPromise: Promise<User> }
			>
		>;
		type _check2 = Assert<
			IsAssignable<
				ReturnType<Methods["createWithRelationships"]>,
				{ readonly runPromise: Promise<User> }
			>
		>;
		type _check3 = Assert<
			IsAssignable<
				ReturnType<Methods["updateWithRelationships"]>,
				{ readonly runPromise: Promise<User> }
			>
		>;
