...
const testData = [
  { id: "1", name: "John Doe", email: "john@company.com", age: 25, role: "admin", active: true, tags: ["a", "b"] },
  { id: "2", name: "Jane Smith", email: "jane@example.com", age: 30, role: "user", active: false, tags: ["b", "c"] },
  { id: "3", name: "John Smith", email: "johns@spam.com", age: 35, role: "admin", active: false, tags: ["a", "c", "d"] },
  { id: "4", name: "Bob Johnson", email: "bob@company.com", age: 17, role: "user", active: true, tags: ["a"] },
  { id: "5", name: "Alice Brown", email: "alice@test.com", age: 22, role: "moderator", active: true, tags: [] },
];
...
describe("applyFilter Stream combinator", () => {
  ...
  describe("$ne operator", () => {
    it("should exclude matching values", async () => {
      const result = await collectFiltered(testData, { role: { $ne: "admin" } });
  ...
  describe("$in operator", () => {
    it("should match values in array", async () => {
      const result = await collectFiltered(testData, { role: { $in: ["admin", "moderator"] } });
      expect(result).toHaveLength(3);
  ...
  describe("$nin operator", () => {
    it("should exclude values in array", async () => {
      const result = await collectFiltered(testData, { role: { $nin: ["admin", "moderator"] } });
      expect(result).toHaveLength(2);
  ...
  describe("$startsWith operator", () => {
    it("should match strings starting with prefix", async () => {
      const result = await collectFiltered(testData, { name: { $startsWith: "John" } });
      expect(result).toHaveLength(2);
      expect(result.map((r) => r.id)).toEqual(["1", "3"]);
    });
  });
  ...
  describe("$endsWith operator", () => {
    it("should match strings ending with suffix", async () => {
      const result = await collectFiltered(testData, { name: { $endsWith: "Smith" } });
      expect(result).toHaveLength(2);
      expect(result.map((r) => r.id)).toEqual(["2", "3"]);
    });
  });
  ...
  describe("$contains operator", () => {
    it("should match strings containing substring", async () => {
      const result = await collectFiltered(testData, { email: { $contains: "@company.com" } });
      expect(result).toHaveLength(2);
      expect(result.map((r) => r.id)).toEqual(["1", "4"]);
    });

    it("should match arrays containing element", async () => {
      const result = await collectFiltered(testData, { tags: { $contains: "a" } });
      expect(result).toHaveLength(3);
  ...
  describe("$all operator", () => {
    it("should match arrays containing all elements", async () => {
      const result = await collectFiltered(testData, { tags: { $all: ["a", "b"] } });
      expect(result).toHaveLength(1);
  ...
  describe("$or operator", () => {
    it("should match items satisfying any condition", async () => {
      const result = await collectFiltered(testData, {
        $or: [{ name: { $startsWith: "John" } }, { email: { $contains: "@company.com" } }],
      });
      expect(result).toHaveLength(3);
      expect(result.map((r) => r.id)).toEqual(["1", "3", "4"]);
    });
  ...
  describe("$not operator", () => {
    it("should exclude items matching the condition", async () => {
      const result = await collectFiltered(testData, { $not: { role: "admin" } });
      expect(result).toHaveLength(3);
      expect(result.map((r) => r.id)).toEqual(["2", "4", "5"]);
    });
  });
  ...
  describe("combined operators", () => {
    it("should combine multiple field filters with AND semantics", async () => {
      const result = await collectFiltered(testData, { role: "admin", active: true });
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe("1");
    });

    it("should combine operator filters on the same field", async () => {
      const result = await collectFiltered(testData, { age: { $gte: 20, $lte: 30 } });
      expect(result).toHaveLength(3);
  ...
  describe("non-existent fields", () => {
    ...
    it("should not match operators on non-existent fields", async () => {
      const result = await collectFiltered(testData, { nonexistent: { $gt: 10 } });
      expect(result).toHaveLength(0);
    });
  });
