			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					return yield* update(
						...
						usersRef,
						stateRefs,
					)("user1", { id: "new-id" } as Record<string, unknown>).pipe(
						Effect.flip,
					);
				}),
			);
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					return yield* update(
						...
						usersRef,
						stateRefs,
					)("user1", { createdAt: "2025-01-01" } as Record<
						string,
						unknown
					>).pipe(Effect.flip);
				}),
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					return yield* update(
						...
						usersRef,
						stateRefs,
					)("user1", { age: "not a number" as unknown as number }).pipe(
						Effect.flip,
					);
				}),
			);
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					return yield* updateMany(
						...
						usersRef,
						stateRefs,
					)(() => true, { id: "new-id" } as Record<string, unknown>).pipe(
						Effect.flip,
					);
				}),
			);
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					return yield* updateMany(
						...
						usersRef,
						stateRefs,
					)((user) => user.id === "user1", { companyId: "nonexistent" }).pipe(
						Effect.flip,
					);
				}),
			);
			...
			const mapAfter = await Effect.runPromise(
				Effect.gen(function* () {
					...
					yield* updateMany(
						...
						usersRef,
						stateRefs,
					)(() => true, { age: "bad" as unknown as number }).pipe(
						Effect.ignore,
					);
