...
describe("Multi-field search", () => {
  it("5.1: terms can span across fields", async () => {
    const db = await Effect.runPromise(createEffectDatabase({
      ...
      books: [
        { id: "1", title: "Dune", author: "Frank Herbert", year: 1965, description: "A desert planet story" },
        { id: "2", title: "Neuromancer", author: "William Gibson", year: 1984, description: "Cyberpunk classic" },
        { id: "3", title: "The Left Hand of Darkness", author: "Ursula K. Le Guin", year: 1969, description: "Gender and society" },
      ],
    }))
  ...
  it("5.3: query tokens can match in different fields", async () => {
    ...
    const result = await db.books.query({
      where: { $search: { query: "le guin darkness", fields: ["title", "author"] } }
    }).runPromise
    expect(result.length).toBe(1)
