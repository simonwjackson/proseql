...
const generateItems = (count: number): ReadonlyArray<Item> => {
	...
	for (let i = 1; i <= count; i++) {
		...
		items.push({
			...
			price: i * 10,
			category:
				i % 3 === 0 ? "electronics" : i % 2 === 0 ? "books" : "clothing",
		});
	}
	return items;
};
...
const runCursorQuery = async (
	...
	return await (
		db.items.query(options) as {
			runPromise: Promise<CursorPageResult<Record<string, unknown>>>;
		}
	).runPromise;
};
...
describe("Cursor Pagination", () => {
	...
	describe("backward pagination", () => {
		...
		it("returns fewer items when near the beginning", async () => {
			...
			expect(page.items.map((i) => i.id)).toEqual(["item-001", "item-002"]);
	...
	describe("combined with populate", () => {
		...
		it("populated fields present in cursor page items via inverse relationship", async () => {
			...
			for (const item of result.items) {
				...
				const products = item.products as ReadonlyArray<
	...
	describe("combined with where", () => {
		it("cursor applies after filtering, correct subset paginated", async () => {
			...
			expect(firstPage.items.every((i) => i.category === "electronics")).toBe(
				true,
			);
			...
			expect(secondPage.items.every((i) => i.category === "electronics")).toBe(
				true,
			);
		...
		it("cursor backward pagination works with where filter", async () => {
			...
			expect(page.items).toHaveLength(2);
			expect(page.items.map((i) => i.id)).toEqual(["item-009", "item-012"]);
	...
	describe("combined with select", () => {
		...
		it("select combined with where and cursor works correctly", async () => {
			...
			expect(result.items).toHaveLength(2);
			expect(result.items.map((i) => i.id)).toEqual(["item-003", "item-006"]);
	...
	describe("implicit sort", () => {
		...
		it("forward pagination works with implicit sort", async () => {
			...
			expect(
				(secondPage.items[0].id as string) > (firstPage.items[2].id as string),
			).toBe(true);
		});
		...
		it("implicit sort combined with where filter", async () => {
			...
			expect(result.items).toHaveLength(2);
			expect(result.items.map((i) => i.id)).toEqual(["item-003", "item-006"]);
			expect(result.items.every((i) => i.category === "electronics")).toBe(
				true,
			);
