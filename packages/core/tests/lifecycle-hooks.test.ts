...
const makeBeforeCreateHook =
	(transform: (data: User) => User): BeforeCreateHook<User> =>
	(ctx) =>
		Effect.succeed(transform(ctx.data));

/**
 * Helper to create a beforeCreate hook that rejects.
 */
const makeRejectingBeforeCreateHook =
	(reason: string): BeforeCreateHook<User> =>
	(ctx) =>
		Effect.fail(
			new HookError({
				hook: "beforeCreate",
 ...
 */
const makeBeforeUpdateHook =
	<T>(transform: (update: T) => T): BeforeUpdateHook<User> =>
	(ctx) =>
		Effect.succeed(transform(ctx.update) as typeof ctx.update);

/**
 * Helper to create a beforeUpdate hook that rejects.
 */
const makeRejectingBeforeUpdateHook =
	(reason: string): BeforeUpdateHook<User> =>
	(ctx) =>
		Effect.fail(
			new HookError({
				hook: "beforeUpdate",
...
const makeRejectingBeforeDeleteHook =
	(reason: string): BeforeDeleteHook<User> =>
	(ctx) =>
		Effect.fail(
			new HookError({
				hook: "beforeDelete",
 ...
 */
const _makePassingBeforeDeleteHook = (): BeforeDeleteHook<User> => () =>
	Effect.void;
...
const makeTrackingAfterCreateHook =
	(calls: Array<AfterCreateContext<User>>): AfterCreateHook<User> =>
	(ctx) => {
		calls.push(ctx);
		return Effect.void;
	};
...
const makeTrackingAfterUpdateHook =
	(calls: Array<AfterUpdateContext<User>>): AfterUpdateHook<User> =>
	(ctx) => {
		calls.push(ctx);
		return Effect.void;
	};
...
const makeTrackingAfterDeleteHook =
	(calls: Array<AfterDeleteContext<User>>): AfterDeleteHook<User> =>
	(ctx) => {
		calls.push(ctx);
		return Effect.void;
	};
...
const makeTrackingOnChangeHook =
	(calls: Array<OnChangeContext<User>>): OnChangeHook<User> =>
	(ctx) => {
		calls.push(ctx);
		return Effect.void;
	};
...
const makeTrackingBeforeCreateHook =
	(calls: Array<BeforeCreateContext<User>>): BeforeCreateHook<User> =>
	(ctx) => {
		calls.push(ctx);
...
const makeTrackingBeforeUpdateHook =
	(calls: Array<BeforeUpdateContext<User>>): BeforeUpdateHook<User> =>
	(ctx) => {
		calls.push(ctx);
...
const makeTrackingBeforeDeleteHook =
	(calls: Array<BeforeDeleteContext<User>>): BeforeDeleteHook<User> =>
	(ctx) => {
		calls.push(ctx);
		return Effect.void;
	};
...
describe("lifecycle-hooks", () => {
	describe("test helpers", () => {
		it("should create a database with hooked collection", async () => {
			const db = await Effect.runPromise(createHookedDatabase({}));
	...
	describe("before hooks", () => {
		...
		it("beforeCreate rejects → create fails with HookError, no state change", async () => {
			const hooks: HooksConfig<User> = {
				beforeCreate: [
					makeRejectingBeforeCreateHook("User creation not allowed"),
				],
			};
					...
					const createResult = yield* db.users
						...
						.pipe(
							Effect.matchEffect({
								...
								onSuccess: (user) =>
									Effect.succeed({ type: "success" as const, user }),
							}),
						);
		...
		it("multiple beforeCreate hooks chain in order", async () => {
			...
			expect(hookOrder[1]).toEqual({
				hookIndex: 2,
				receivedName: "Original-hook1",
			});
			expect(hookOrder[2]).toEqual({
				hookIndex: 3,
				receivedName: "Original-hook1-hook2",
			});
		...
		it("beforeDelete rejects → delete fails, entity still exists", async () => {
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					...
					const deleteResult = yield* db.users.delete("u1").pipe(
						Effect.matchEffect({
							onFailure: (error) =>
								Effect.succeed({ type: "error" as const, error }),
							onSuccess: (user) =>
	...
	describe("after hooks", () => {
		...
		it("after-hook error does not fail the CRUD operation", async () => {
			...
			const _failingAfterCreateHook: AfterCreateHook<User> = () =>
				Effect.fail(new Error("afterCreate hook failed!"));

			const _failingAfterUpdateHook: AfterUpdateHook<User> = () =>
				Effect.fail(new Error("afterUpdate hook failed!"));

			const _failingAfterDeleteHook: AfterDeleteHook<User> = () =>
					...
					const findAfterDelete = yield* db.users.findById(created.id).pipe(
						Effect.matchEffect({
							onFailure: (error) =>
								Effect.succeed({ type: "error" as const, error }),
							onSuccess: (user) =>
								Effect.succeed({ type: "found" as const, user }),
						}),
					);
		...
		it("multiple after-hooks run in order", async () => {
			...
			const makeOrderedAfterCreateHook =
				(index: number): AfterCreateHook<User> =>
				() => {
					createHookOrder.push(index);
					return Effect.void;
				};

			const makeOrderedAfterUpdateHook =
				(index: number): AfterUpdateHook<User> =>
				() => {
					updateHookOrder.push(index);
					return Effect.void;
				};

			const makeOrderedAfterDeleteHook =
				(index: number): AfterDeleteHook<User> =>
				() => {
		...
		it("after-hooks run after state mutation is complete", async () => {
			...
			let dbRef: Awaited<
				ReturnType<
					typeof Effect.runPromise<ReturnType<typeof createHookedDatabase>>
				>
			> | null = null;

			// Track what we find when querying the database from inside hooks
			const afterCreateFindings: Array<{
				found: boolean;
				entity: User | null;
			}> = [];
			const afterUpdateFindings: Array<{
				found: boolean;
				entity: User | null;
				nameUpdated: boolean;
			}> = [];
			...
			expect(afterUpdateFindings[0].entity?.name).toBe(
				"Updated State Test User",
			);
	...
	describe("onChange hooks", () => {
		...
		it("onChange works alongside specific hooks (both fire)", async () => {
			...
			const beforeUpdateHook: BeforeUpdateHook<User> = (ctx) => {
				beforeUpdateCalls.push({
					existing: ctx.existing.name,
					update: ctx.update,
				});
				return Effect.succeed(ctx.update);
			};
			...
			const afterUpdateHook: AfterUpdateHook<User> = (ctx) => {
				afterUpdateCalls.push({
					previous: ctx.previous.name,
	...
	describe("batch and upsert", () => {
		it("createMany: hooks run per entity", async () => {
			...
			expect(
				onChangeCalls
					.filter(
						(c): c is Extract<OnChangeContext<User>, { type: "create" }> =>
							c.type === "create",
					)
					.map((c) => c.entity.name),
			).toEqual([
		...
		it("updateMany: hooks run per entity", async () => {
			...
			const updateContexts = onChangeCalls.filter(
				(c): c is Extract<OnChangeContext<User>, { type: "update" }> =>
					c.type === "update",
			);
		...
		it("deleteMany: hooks run per entity", async () => {
			...
			const deleteContexts = onChangeCalls.filter(
				(c): c is Extract<OnChangeContext<User>, { type: "delete" }> =>
					c.type === "delete",
			);
		...
		it("upsert update path: triggers beforeUpdate/afterUpdate/onChange('update')", async () => {
			...
			const testData = {
				users: [{ id: "u1", name: "Alice", email: "alice@test.com", age: 30 }],
			};
			...
			expect(beforeUpdateCalls[0].update).toEqual({
				name: "UpdatedAlice",
				age: 31,
			});
