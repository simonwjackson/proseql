...
describe("CRUD Batch Operations and Edge Cases (Effect-based)", () => {
	...
	describe("Large batch operations", () => {
		...
		it("should handle large updateMany operations", async () => {
			...
			expect(result.updated.every((u) => u.tags?.includes("processed"))).toBe(
				true,
			);
		...
		it("should handle large upsertMany operations", async () => {
			...
			expect(result.updated.every((u) => u.tags?.includes("updated"))).toBe(
				true,
			);
		});
	});
	...
	describe("Special field handling", () => {
		...
		it("should handle boolean operations correctly", async () => {
			...
			result = await db.companies.update("comp1", { isActive: { $set: false } })
				.runPromise;
			expect(result.isActive).toBe(false);
		});

		it("should handle numeric edge values", async () => {
			let result = await db.companies.update("comp1", { revenue: { $set: 0 } })
				.runPromise;
			expect(result.revenue).toBe(0);

			result = await db.companies.update("comp1", { revenue: { $set: -1000 } })
				.runPromise;
