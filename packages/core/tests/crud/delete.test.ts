	...
	describe("delete method (single entity)", () => {
		describe("hard delete", () => {
			it("should delete entity and return it", async () => {
				...
				expect(
					users.find((u: Record<string, unknown>) => u.id === "user2"),
				).toBeUndefined();
			});
		...
		describe("soft delete", () => {
			it("should soft delete entity with deletedAt field", async () => {
				await db.comments.deleteMany(
					(c) => c.postId === "post1" || c.postId === "post3",
				).runPromise;
				...
				const deletedUser = allUsers.find(
			...
			it("should soft delete posts", async () => {
				...
				expect(result.deletedAt).toBeDefined();
				expect(new Date(result.deletedAt!).getTime()).toBeGreaterThanOrEqual(
			...
			it("should fail soft delete on entity without deletedAt field", async () => {
				...
				const categories = await db.categories.query().runPromise;
				expect(
					categories.find((c: Record<string, unknown>) => c.id === "cat1"),
				).toBeDefined();
			});
		});
	...
	describe("deleteMany method (batch delete)", () => {
		describe("basic batch deletion", () => {
			it("should delete all matching entities", async () => {
				const result = await db.comments.deleteMany((c) => c.flagged === true)
					.runPromise;
			...
			it("should delete with OR-like conditions", async () => {
				...
				expect(result.deleted.map((p) => p.id).sort()).toEqual([
					"post2",
					"post3",
				]);
		...
		describe("batch soft delete", () => {
			...
			it("should handle mixed soft delete capability", async () => {
				const error = await Effect.runPromise(
					db.logs
						.deleteMany((l) => l.level === "error", { soft: true })
						.pipe(Effect.flip),
				);
		...
		describe("batch delete with limit", () => {
			it("should respect limit option", async () => {
				const result = await db.comments.deleteMany(() => true, { limit: 2 })
					.runPromise;
			...
			it("should handle limit with soft delete", async () => {
				...
				const result = await db.posts.deleteMany(() => true, {
					soft: true,
					limit: 1,
				}).runPromise;
		...
		describe("batch delete error handling", () => {
			it("should handle empty result gracefully", async () => {
				const result = await db.posts.deleteMany((p) =>
					p.title.includes("NonExistent"),
				).runPromise;
			...
			it("should handle negative limit by deleting all", async () => {
				const result = await db.logs.deleteMany(() => true, { limit: -1 })
					.runPromise;
				expect(result.count).toBe(3);
			});
		});
