...
describe("Persistence System (Effect-based)", () => {
	describe("Basic Persistence", () => {
		...
		it("should persist and reload data correctly (round-trip)", async () => {
			...
			const result = await Effect.runPromise(
				Effect.provide(
					Effect.gen(function* () {
						...
						const loadedUsers = yield* loadData(
							"/test-data/users.json",
							UserSchema,
						);
						const loadedPosts = yield* loadData(
							"/test-data/posts.json",
							PostSchema,
						);
			...
			expect(result.posts.get("post-1")?.title).toBe(
					...
					Effect.gen(function* () {
						yield* saveCollectionsToFile("/test-data/content.json", [
							{ name: "posts", schema: PostSchema, data: samplePosts },
							{
								name: "categories",
								schema: CategorySchema,
								data: new Map([["cat-1", { id: "cat-1", name: "Technology" }]]),
							},
						]);
			...
			await Effect.runPromise(
				Effect.provide(
					Effect.gen(function* () {
						...
						expect(yield* storage.exists("/test-data/sessions.json")).toBe(
							false,
						);
			...
			const result = await Effect.runPromise(
				Effect.provide(
					Effect.gen(function* () {
						...
						const sessionsRef = yield* Ref.make<ReadonlyMap<string, unknown>>(
							new Map(),
						);
	...
	describe("Error Handling", () => {
		...
		it("should handle storage write errors", async () => {
			...
			const error = await Effect.runPromise(
				Effect.provide(
					saveData("/test-data/users.json", UserSchema, sampleUsers).pipe(
						Effect.flip,
					),
					layer,
				),
	...
	describe("Backward Compatibility (pure in-memory)", () => {
		it("should work with in-memory Ref state without persistence", async () => {
			...
			const result = await Effect.runPromise(
				Effect.gen(function* () {
					const usersRef =
						yield* Ref.make<ReadonlyMap<string, User>>(sampleUsers);
					const postsRef =
						yield* Ref.make<ReadonlyMap<string, Post>>(samplePosts);
	...
	describe("Layer Swapping", () => {
		it("same program runs against different serializer layers", async () => {
			...
			expect(jsonResult.get("user-1")?.name).toBe(
				yamlResult.get("user-1")?.name,
			);
		});
