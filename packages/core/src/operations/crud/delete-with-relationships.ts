...
const processRelationshipCascades = <T extends HasId>(
	...
	options:
		| DeleteWithRelationshipsOptions<T, Record<string, RelationshipDef>>
		| undefined,
): Effect.Effect<
	{
...
> =>
	Effect.gen(function* () {
		...
		for (const [field, relationship] of Object.entries(relationships)) {
			...
			const foreignKey = findForeignKey(
				relationship,
				field,
				collectionName,
				targetConfig,
			...
			const cascadeOption: CascadeOption =
				(options?.include as Record<string, CascadeOption> | undefined)?.[
					field
				] || "preserve";

			const relatedEntities = yield* findRelatedEntities(
				entityId,
				foreignKey,
				targetRef,
			);
 ...
 */
export const deleteWithRelationships =
	<T extends HasId>(
		collectionName: string,
		relationships: Record<string, RelationshipConfig>,
	...
	> =>
		Effect.gen(function* () {
			...
			const { restrictViolations, cascadeResults } =
				yield* processRelationshipCascades(
					id,
					collectionName,
					relationships,
					stateRefs,
 ...
 */
export const deleteManyWithRelationships =
	<T extends HasId>(
		collectionName: string,
		relationships: Record<string, RelationshipConfig>,
		ref: Ref.Ref<ReadonlyMap<string, T>>,
	...
	> =>
		Effect.gen(function* () {
			...
			// 4. Process cascade operations for all entities
			const allCascadeResults: Record<
				string,
