...
- [ ] 2.1 Create `core/reactive/change-pubsub.ts` with `createChangePubSub()`: returns `Effect.Effect<PubSub.PubSub<ChangeEvent>>` using `PubSub.unbounded()`
...
- [ ] 3.1 Create `core/reactive/watch.ts` with `watch(pubsub, ref, collectionName, config)`: subscribe to PubSub, filter by collection, debounce, re-evaluate query, deduplicate, emit as Stream
- [ ] 3.2 Initial emission: the stream emits the current result set immediately upon subscription (before waiting for any change events)
...
- [ ] 4.1 Extract a reusable `evaluateQuery(ref, config)` function that reads the current Ref state and runs the query pipeline (filter, sort, select, limit/offset)
...
- [ ] 5.1 Add optional `changePubSub?: PubSub.PubSub<ChangeEvent>` parameter to `create` in `core/operations/crud/create.ts`. After state mutation and hooks, publish `ChangeEvent` with `operation: "create"`.
- [ ] 5.2 Same for `createMany`: publish a single event after all entities are inserted (not per entity)
- [ ] 5.3 Add `changePubSub` parameter to `update` in `core/operations/crud/update.ts`. Publish with `operation: "update"` after mutation.
- [ ] 5.4 Same for `updateMany`: publish a single event after all updates
...
- [ ] 7.1 Modify `createTransaction` in `core/transactions/transaction.ts` to accept `changePubSub` parameter
...
- [ ] 9.1 Create `core/reactive/watch-by-id.ts` implementing `watchById(pubsub, ref, collectionName, id)` as a thin wrapper over `watch()` with `where: { id }`, mapping results to `T | null`
...
- [ ] 12.1 Create `tests/reactive-queries.test.ts` with test helpers: database with a hooked collection, PubSub, and Scope
...
- [ ] 13.1 Test create triggers watch: inserting a matching entity causes a new emission with the entity included
