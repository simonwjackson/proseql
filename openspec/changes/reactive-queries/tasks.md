...
- [x] 13.3 Test delete triggers watch: deleting a matched entity causes a new emission without the entity
- [x] 13.4 Test entity entering result set: updating a non-matching entity to match the where clause triggers emission
- [x] 13.5 Test entity leaving result set: updating a matching entity to no longer match triggers emission

## 14. Tests — Irrelevant Mutations

- [x] 14.1 Test mutation to a different collection does not trigger re-evaluation
- [x] 14.2 Test mutation that does not change the result set (e.g., creating a non-matching entity) does not produce a new emission (deduplication)
...
- [x] 15.3 Test transaction rollback: no emissions (state unchanged)

## 16. Tests — File Changes

- [x] 16.1 Test file watcher reload triggers watch emission when result set changes
- [x] 16.2 Test file watcher reload does not emit when result set is unchanged (deduplication)

## 17. Tests — Debounce

- [x] 17.1 Test rapid mutations (e.g., 50 creates in a loop) produce at most one emission after debounce settles
- [x] 17.2 Test configurable debounce interval is respected

## 18. Tests — Unsubscribe and Cleanup

- [x] 18.1 Test stream interruption stops emissions and cleans up the PubSub subscription
- [x] 18.2 Test Scope closure cleans up all active subscriptions
- [x] 18.3 Test `watchById` emits entity state, re-emits on update, emits null on deletion
