...
- [ ] 1.1 Create `rpc/src/rpc-errors.ts`: define Effect Schemas for each proseql tagged error (`NotFoundError`, `ValidationError`, `DuplicateKeyError`, `UniqueConstraintError`, `ForeignKeyError`, `HookError`, `TransactionError`, `DanglingReferenceError`) so they can be serialized/deserialized across RPC transport
...
- [ ] 3.5 Wire `update` handler: delegate to `db[collection].update(id, updates)`, propagate typed errors
- [ ] 3.6 Wire `delete` handler: delegate to `db[collection].delete(id)`, propagate typed errors
...
- [ ] 5.1 Create `rest/src/handlers.ts` with `createRestHandlers(config, db)` returning an array of `{ method, path, handler }` route descriptors
...
- [ ] 7.1 Implement `GET /:collection` handler: parse query params, delegate to `db[collection].query(config)`, collect results, return `{ status: 200, body: results }`
- [ ] 7.2 Implement `GET /:collection/:id` handler: delegate to `db[collection].findById(id)`, return 200 or mapped error
...
- [ ] 9.1 Create `rest/src/relationship-routes.ts` that inspects collection relationships in the config
...
- [ ] 10.1 Create `rpc/tests/rpc-group.test.ts`: verify makeRpcGroup produces correct procedures for a single-collection config
...
- [ ] 11.1 Create `rest/tests/query-params.test.ts`: test simple equality parsing
...
- [ ] 11.13 Test PUT updates entity and returns 200
- [ ] 11.14 Test DELETE removes entity and returns 200
