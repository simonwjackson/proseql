...
- [x] 1.2 Create `tsconfig.base.json` at root with shared compiler options: `target: "ES2022"`, `module: "esnext"`, `moduleResolution: "bundler"`, `strict: true`, `declaration: true`, `declarationMap: true`, `composite: true`, `skipLibCheck: true`. Add `paths` mapping `@proseql/core` → `./packages/core/src/index.ts`
- [x] 1.3 Delete old `tsconfig.json` and `tsconfig.test.json` (replaced by base + per-package configs)
...
- [x] 2.4 Create `packages/core/package.json`: name `@proseql/core`, version `0.1.0`, type `module`, main `src/index.ts`, dependencies: `effect`, `yaml`, `json5`, `jsonc-parser`, `smol-toml`, `@toon-format/toon`, `hjson`
...
- [x] 2.9 Move `tests/crud/` subdirectory to `packages/core/tests/crud/`, update imports within
...
- [x] 4.3 Create `packages/rest/package.json`: name `@proseql/rest`, version `0.0.0`, type `module`, main `src/index.ts`, dependencies: `{ "@proseql/core": "workspace:*" }`, peerDependencies: `{ "effect": "^3.15.0" }`
- [x] 4.4 Create `packages/rest/tsconfig.json`: extends `../../tsconfig.base.json`, rootDir `src`, outDir `dist`, references `[{ "path": "../core" }]`
...
- [x] 5.4 Create `packages/rpc/tsconfig.json`: extends `../../tsconfig.base.json`, rootDir `src`, outDir `dist`, references `[{ "path": "../core" }]`
...
- [x] 7.5 Create `packages/node/default.nix`: `mkBunDerivation` with `pname = "proseql-node"`
- [x] 7.6 Create `packages/rest/default.nix`: `mkBunDerivation` with `pname = "proseql-rest"`
- [x] 7.7 Create `packages/rpc/default.nix`: `mkBunDerivation` with `pname = "proseql-rpc"`
- [x] 7.8 Delete old `shell.nix` (replaced by `flake.nix` devShell)
- [x] 7.9 Run `bun install` and verify `bun.nix` is generated by postinstall hook

## 8. Cleanup & References

- [x] 8.1 Delete old `core/` directory (all content moved to packages)
- [x] 8.2 Delete old `tests/` directory (all content moved to packages)
- [x] 8.3 Update `examples/*.ts` imports: change `../core/...` to `@proseql/core` or `@proseql/node`
- [x] 8.4 Update `CLAUDE.md`: change `core/` references to `packages/core/src/`, update test commands, add workspace context
