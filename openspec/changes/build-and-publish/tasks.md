...
- [x] 4.2 Verify `@proseql/core` dependency uses `"workspace:*"` (already does).
- [x] 4.3 Add `effect` as a `peerDependency` if not already present (node re-exports core which depends on effect).

## 5. Per-Package package.json -- @proseql/rest

- [x] 5.1 Apply the same field changes as core.
- [x] 5.2 Bump version from `0.0.0` to `0.1.0`.
- [x] 5.3 Verify `peerDependencies` includes `effect`.

## 6. Per-Package package.json -- @proseql/rpc

- [x] 6.1 Apply the same field changes as core.
- [x] 6.2 Bump version from `0.0.0` to `0.1.0`.
- [x] 6.3 Verify `peerDependencies` includes `effect` and `@effect/rpc`.
...
- [x] 9.2 Verify `package.json` in the tarball has no `workspace:*` references (use `bun publish --dry-run` or `npm pack` + inspect).
- [x] 9.3 Verify tarball size is reasonable (no `src/`, `tests/`, `node_modules/`, or `.ts` source files).
