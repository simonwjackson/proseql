...
- [ ] 1.1 Create `packages/core/src/serializers/codecs/prose.ts` with types: `ProseSegment` (field | literal), `CompiledTemplate`, `ProseCodecOptions`
...
- [ ] 2.1 Implement `serializeValue(value: unknown): string` — numbers as digits, booleans as `true`/`false`, null as `~`, arrays as `[a, b, c]`, strings as bare text
