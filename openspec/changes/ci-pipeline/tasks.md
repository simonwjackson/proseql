...
- [ ] 1.2 Define `test` job: runs-on `ubuntu-latest`, strategy matrix with Bun versions (`latest`, `1.2`), steps: checkout, setup-bun (with cache enabled), `bun install`, `bun test`
...
- [ ] 2.1 Create `.github/workflows/publish.yml` with trigger configuration: `on: push: tags: ['v*']`
- [ ] 2.2 Define `test`, `typecheck`, and `lint` jobs identical to the CI workflow (or use a reusable workflow reference)
