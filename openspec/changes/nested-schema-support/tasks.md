...
- [x] 2.3 Replace local `getNestedValue` in `core/operations/query/cursor-stream.ts` with import from `core/utils/nested-path.ts`. Remove the local function.
- [x] 2.4 Run existing tests (`just test-core`) to confirm no regressions from the import change.
...
- [x] 3.10 Test 3-level nesting: `where: { a: { b: { c: { $eq: "deep" } } } }`.
- [x] 3.11 Test mixed flat + nested in same where: `where: { title: "Dune", metadata: { views: { $gt: 100 } } }`.
- [x] 3.12 Test nested `$search` field resolution: `where: { $search: { query: "hello", fields: ["metadata.description"] } }`.
...
- [x] 4.11 Test that flat schemas (no nested objects) behave identically to before (regression).
...
- [x] 8.4 Write type-level tests: verify TypeScript accepts nested where clauses, nested update operators, dot-path index declarations, and dot-path aggregate field refs. Verify TypeScript rejects invalid nested paths.
