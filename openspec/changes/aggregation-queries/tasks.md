...
- [ ] 1.2 Add `aggregate` method signature to `SmartCollection` in `core/types/types.ts`. Generic over config, return type branches on `groupBy` presence: `RunnableEffect<GroupedAggregateResult>` or `RunnableEffect<AggregateResult>`
