...
- [x] 2.2 Implement `buildIndexes(normalizedIndexes, initialData)` returning `Effect<CollectionIndexes>`. For each index, scan all entities and build the `Map<key, Set<id>>`. Single-field keys use the raw value. Compound keys use `JSON.stringify(fields.map(f => entity[f]))`. Skip null/undefined values.
...
- [x] 5.4 Default to empty `CollectionIndexes` when `indexes` is not configured (preserves existing behavior).
...
- [x] 6.3 Test multiple entities with same field value: all IDs in the index Set
- [x] 6.4 Test null/undefined values not indexed
- [x] 6.5 Test collection without indexes: empty CollectionIndexes
...
- [x] 8.8 Test result parity: same results with and without index configured
...
- [x] 10.2 Run type check (`bunx tsc --noEmit`) to verify no type errors
