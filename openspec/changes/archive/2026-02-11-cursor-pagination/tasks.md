...
- [x] 3.2 Implement cursor pipeline branch: filter → populate → sort → collect via `applyCursor` → apply select to collected items → package as `CursorPageResult` → wrap with `RunnableCursorPage` (`.runPromise`)
- [x] 3.3 Ensure select is applied after cursor value extraction so projected items don't lose cursor key data
- [x] 3.4 Update `queryFn` options type to accept `cursor?: CursorConfig` (runtime), relying on `QueryConfig` type union for compile-time mutual exclusivity with `limit`/`offset`
...
- [x] 4.2 Test forward pagination: first page returns correct items and `hasNextPage = true`, second page via `after: endCursor` returns next items, final page has `hasNextPage = false`
- [x] 4.3 Test backward pagination: page via `before` cursor returns previous items, first page has `hasPreviousPage = false`
...
- [x] 4.11 Test validation errors: both `after` + `before` → `ValidationError`, invalid key → `ValidationError`, `limit <= 0` → `ValidationError`, sort mismatch → `ValidationError`
- [x] 4.12 Run full test suite (`bun test`) to verify no regressions in existing offset/limit pagination
