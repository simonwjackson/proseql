...
- [ ] 1.1 Create `packages/cli/package.json` with name `@proseql/cli`, `bin: { proseql: "./src/main.ts" }`, dependencies on `@proseql/core` and `@proseql/node`
- [ ] 1.2 Create `packages/cli/tsconfig.json` extending root tsconfig
- [ ] 1.3 Add `packages/cli` to the root workspace configuration
- [ ] 1.4 Create `packages/cli/src/main.ts` entry point: parse top-level flags (`--config`, `--help`, `--version`, `--json`, `--yaml`, `--csv`), dispatch to command handlers based on first positional argument
- [ ] 1.5 Verify `bun run packages/cli/src/main.ts --help` executes without error

## 2. Config Discovery

- [ ] 2.1 Create `packages/cli/src/config/discovery.ts`: `discoverConfig(cwd, overridePath?)` walks from `cwd` upward, checking for `proseql.config.ts`, `proseql.config.js`, `proseql.config.json` in order. Returns absolute path or fails with descriptive error.
...
- [ ] 3.1 Create `packages/cli/src/commands/init.ts`: check for existing config in cwd, abort with warning if found
- [ ] 3.2 Scaffold `proseql.config.ts` with an example collection definition
- [ ] 3.3 Create `data/` directory with an example data file in the chosen format (default JSON, override with `--format`)
...
- [ ] 6.1 Create `packages/cli/src/commands/create.ts`: parse `--data` as JSON string, call `create` on the collection, print the created entity
