...
- [x] 6.5 Create `packages/cli/src/commands/delete.ts`: resolve collection and entity ID, prompt for confirmation (unless `--force`), call `delete`, print confirmation message
...
- [x] 7.3 Implement `migrate --dry-run`: show what migrations would run without executing them
- [x] 7.4 Implement `migrate` (run): prompt for confirmation (unless `--force`), execute all pending migrations, report results
...
- [x] 8.2 Write the new data file with the correct extension, remove the old file
- [x] 8.3 Update the config file to reference the new file path/format
- [x] 8.4 Print summary of conversion (old format, new format, file paths)
...
- [x] 10.6 Create `packages/cli/tests/commands/query.test.ts`: test basic query, filtered query, select/sort/limit, output format flags
