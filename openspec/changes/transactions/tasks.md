...
- [ ] 1.1 Update `TransactionContext` in `core/types/crud-types.ts`: change `rollback` to `() => Effect<never, TransactionError>`, `commit` to `() => Effect<void, TransactionError>`, add `readonly mutatedCollections: ReadonlySet<string>`, add collection accessor index signature
- [ ] 1.2 Add `$transaction` method signature to the database type in `core/types/types.ts`: `$transaction<A, E>(fn: (ctx: TransactionContext) => Effect<A, E>): Effect<A, E | TransactionError>`
...
- [ ] 2.5 Implement `rollback()`: verify `isActive`, restore each collection Ref to its snapshot via `Ref.set`, release lock, mark inactive. Return `Effect.fail(TransactionError { operation: "rollback" })` to short-circuit.
...
- [ ] 3.1 Implement `$transaction(fn)` in `core/transactions/transaction.ts`: create transaction → run `fn(ctx)` → on success call `ctx.commit()` → on failure call `ctx.rollback()` then re-raise the original error.
...
- [ ] 4.1 Add `transactionLock: Ref<boolean>` to `createEffectDatabase` in `core/factories/database-effect.ts`. Initialize as `Ref.make(false)`.
- [ ] 4.2 Create a `buildCollectionForTx` function that mirrors `buildCollection` but accepts a transaction-aware `afterMutation` that records mutations to the transaction's set instead of scheduling persistence.
...
- [ ] 5.1 Create `tests/transactions.test.ts` with test helpers: create a multi-collection database (users + posts with relationships)
...
- [ ] 7.1 Test read-own-writes: create entity within transaction, query sees it immediately
...
- [ ] 8.1 Test no persistence writes during active transaction (use a spy/mock on persistence trigger)
- [ ] 8.2 Test batch persistence on commit: single save triggered per mutated collection
