...
- [ ] 5.1 Add an optional `customOperators?: Map<string, CustomOperator>` parameter to `matchesFilter` in `core/types/operators.ts`. When a `$`-prefixed key is not recognized as a built-in operator, look it up in the custom operators map and call `evaluate(fieldValue, operand)`.
- [ ] 5.2 Extend `isFilterOperatorObject` to recognize custom operator keys (any `$`-prefixed key present in the custom operators map).
...
- [ ] 6.1 In `create` (`core/operations/crud/create.ts`), when the collection config specifies `idGenerator` and the input has no `id` field, look up the generator by name from the resolved ID generator map and call `generate()` to produce the ID.
- [ ] 6.2 Pass the ID generator map from `buildCollection` to `create` and `createMany`.
...
- [ ] 8.1 Add optional `options?: { plugins?: ReadonlyArray<ProseQLPlugin> }` parameter to `createEffectDatabase`.
...
- [ ] 9.5 Test plugin with missing name fails with PluginError
- [ ] 9.6 Test plugin with malformed operator (missing evaluate) fails with PluginError
...
- [ ] 12.1 Test custom `$regex` operator: `where: { title: { $regex: "^The.*" } }` matches correctly
