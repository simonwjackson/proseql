...
- [x] 8.4 Implement `--json` flag: output all results as a structured JSON object keyed by suite and benchmark name
- [x] 8.5 Implement suite filtering: allow running a single benchmark file via argument (e.g., `bun run bench scaling`)

## 9. Tests and Verification

- [x] 9.1 Verify data generators produce correct entity counts and field shapes
- [x] 9.2 Verify the benchmark runner discovers and executes all bench files
- [x] 9.3 Verify JSON output is valid and contains expected keys
- [x] 9.4 Verify table output renders without errors
- [x] 9.5 Run the full benchmark suite end-to-end and confirm all suites complete without failures
...
- [x] 10.3 Run lint (`biome check .`) to verify benchmark files conform to project style
